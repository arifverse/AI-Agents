...
const calcDistance = (start, task) => {
  const startEdge = start.slot <= Math.ceil(NUM_SLOTS/2) ? 0 : NUM_SLOTS + 1;
  const targetEdge = task.slot <= Math.ceil(NUM_SLOTS/2) ? 0 : NUM_SLOTS + 1;
  return Math.abs(start.aisle - task.aisle) + Math.abs(startEdge - targetEdge) + Math.abs(targetEdge - task.slot);
};

export default function PutawayHelper() {
  const [putaways, setPutaways] = useState([]);
  const [currentLocation, setCurrentLocation] = useState({ aisle: 1, slot: 1 });

  useEffect(() => {
    const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    const mockPutaways = Array.from({ length: 10 }, (_, i) => ({
      id: i + 1,
    ...
    const sorted = mockPutaways.sort((a, b) => {
  ...
  const handlePutawayComplete = (id) => {
    const completed = putaways.find(p => p.id === id);
    if (completed) {
      setCurrentLocation({ aisle: completed.aisle, slot: completed.slot });
    }

    const remaining = putaways.filter(p => p.id !== id);
  ...
  const renderMap = () => {
    const route = [currentLocation, ...putaways];
    const grid = [];
    ...
    for (let a = 1; a <= NUM_AISLES; a++) {
      ...
      for (let s = 1; s <= NUM_SLOTS; s++) {
        ...
        const taskIndex = putaways.findIndex(p => p.aisle === a && p.slot === s);
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <div className="flex gap-2">
        {/* Left half: Putaway List */}
        <div className="flex-1">
          <div className="bg-white rounded-2xl shadow p-4">
            <div className="flex justify-center mb-2">
              <h1 className="text-2xl font-bold">Order of Putaways</h1>
            </div>
            {putaways.map((p, index) => (
              <div key={p.id} className="border-b py-2 flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={() => handlePutawayComplete(p.id)}
                  >
                    Complete
                  </button>
                </div>
            ...
            {putaways.length === 0 && <p className="text-center text-gray-500">All putaways completed!</p>}
          </div>
        </div>
